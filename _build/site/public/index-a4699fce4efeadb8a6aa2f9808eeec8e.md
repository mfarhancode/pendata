...
In this implementation, I focused on calculating the dissimilarity between instances of the Iris dataset using numeric distance metrics[cite: 57, 60]. The dataset contains 150 instances and 4 numeric variables.
...
* **Metric Selection**: I selected the **Euclidean Distance ($h=2$)** metric, which is a special case of the **Minkowski Distance**[cite: 73, 80]. This measures the "straight-line" difference between flowers based on their physical dimensions[cite: 73].
* **Matrix Generation**: The result is a **Dissimilarity Matrix** where each cell $d(i, j)$ represents the calculated difference between two specific Iris flowers[cite: 17, 18, 58].
...
* **Grouping**: The "branches" of the tree show how flowers with the lowest dissimilarity (closest to 0) are joined together first[cite: 11, 66].
* **Species Separation**: I observed that flowers of the same species naturally clustered on the same branches, while different species were separated by larger "distances" in the tree structure.
* **Clustering Result**: This visual tool confirms that the four numeric measurements are effective features for distinguishing between the different Iris species.